...
> 旧名 `genMemoizedFetch` 仍可使用，但建议迁移到 `createCachedRequest`。
...
- 缓存超时后自动清除，下次调用重新请求。
- 旧名 `genMemoizedFetch` 仍可用（`import { genMemoizedFetch } from '@fexd/tools'`），但标记为 deprecated。

## 另见

- [`singleflight`](./singleflight) — 并发请求合并（singleflight 模式）
- [`memoize`](../函数/memoize) — 函数结果缓存
