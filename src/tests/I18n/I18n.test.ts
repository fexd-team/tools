...
describe('I18n', () => {
  ...
  describe('静态方法 - template', () => {
    ...
    test('应该正确替换多个变量', () => {
      const result = I18n.template('{{greeting}} {{name}}, you are {{age}} years old', {
        greeting: 'Hello',
        name: 'CJY',
        age: 25,
      })
    ...
    test('split=true 时应返回数组', () => {
      const result = I18n.template('hello {{name}}', { name: 'CJY' }, { split: true })
  ...
  describe('translate - splitByDot 选项', () => {
    ...
    test('splitByDot=true 应强制使用点分割', () => {
      const result = i18n.t('user.name', { '@splitByDot': true })
      expect(result).toBe('点分割方式')
    })

    test('splitByDot=false 应强制使用嵌套对象', () => {
  ...
  describe('性能和内存', () => {
    ...
    test('每个实例应有唯一的 key', () => {
      ...
      // 虽然可能重复（random），但我们测试它们被设置了
      expect(typeof i18n1.key).toBe('number')
      expect(typeof i18n2.key).toBe('number')
      expect(typeof i18n3.key).toBe('number')
    })
  })
            ...
            format: (str, options) => {
              ...
              for (const key in options) {
                ...
                processedOptions[key] =
                  typeof value === 'string' && i18n.resources.default['zh-CN'][value]
